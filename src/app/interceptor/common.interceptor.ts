...
export const CommonInterceptor: HttpInterceptorFn = (req: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> => {
  ...
  return next(req).pipe(
    tap(event => {
      if (event instanceof HttpResponse && event.body) {
        const body = event.body as { statusCode: number, message: string, result: any };
